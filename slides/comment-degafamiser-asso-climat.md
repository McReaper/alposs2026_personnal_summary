# Comment déGAFAMiser une asso climat ?

## Intervenant
Gauthier Fanucci · Alternatiba

## Société
Alternatiba est un mouvement citoyen français pour le climat et la justice sociale, structuré en une centaine de groupes locaux. L'association est connue pour ses villages des alternatives, ses tours à vélo et ses marches pour le climat. Elle fonctionne essentiellement sur des ressources bénévoles et des subventions associatives, avec une infrastructure numérique gérée par deux responsables IT bénévoles. Son engagement militant la pousse à privilégier les logiciels libres pour la gestion de ses données, notamment celles de ses militants.

## Résumé
Alternatiba sort des GAFAM par étapes : mails migrés sur Grésille, données militantes sur Baserow + N8N auto-hébergés, Drive vers Nextcloud en cours. Le mécénat de compétences (coopérative Télescope) est le modèle qui rend ces migrations accessibles à une asso bénévole.

## Points marquants
- Alternatiba utilise encore Google Drive en 2026.
- Mécénat de compétences : modèle viable pour associations en transition.
- Critère hébergeur : "peut-on aller boire un coup avec lui ?"
- Fin Windows 10 utilisée comme levier grand public vers le libre.

## Technologies
- **Baserow** — Base de données no-code open source, alternative à Airtable et Google Sheets. Hébergeable en propre, adapté aux structures qui veulent maîtriser leurs données sans compétences SQL avancées. Pertinent pour les ESN proposant des solutions souveraines aux associations et PME.
- **N8N** — Outil d'automatisation de workflows open source, concurrent de Zapier et Make. Auto-hébergeable, il permet des intégrations API et des automatisations métier sans dépendance SaaS. Très utilisé dans des contextes d'intégration légère ou de RPA.
- **Nextcloud** — Suite collaborative open source (stockage, agenda, documents). Standard de fait pour la souveraineté des données en entreprise et collectivités. Forte demande en déploiement et maintien en condition opérationnelle.
- **Framasoft / les Chatons** — Réseau d'hébergeurs éthiques proposant des alternatives libres aux GAFAM. Cadre de référence pour identifier des partenaires d'hébergement conformes RGPD et à valeurs compatibles.
- **Grésille** — Association à but non lucratif proposant des services numériques libres (messagerie, stockage, listes de diffusion, pads) sur un modèle de contribution libre. Fonctionne en autonomie régionale avec des permanences ; alternative éthique et transparente à OVH ou Gmail pour associations et militants.
